...
_EMPTY = {
    ...
    "macd": None, "macd_signal_line": None, "macd_histogram": None, "macd_signal": "Unknown",
    "adx": None, "adx_signal": "Unknown",
    "atr": None,
    "bb_upper": None, "bb_middle": None, "bb_lower": None, "bb_signal": "Unknown",
    "ema_20": None, "ema_50": None, "ema_200": None, "ema_signal": "Unknown",
    "volume_sma_20": None,
...
def _try_jugaad(symbol: str) -> pd.DataFrame:
    ...
    try:
        ...
        if required.issubset(df.columns) and len(df) >= 50:
...
def _macd_signal(macd, sig):
    if macd is None or sig is None: return "Unknown"
...
def _bb_signal(price, upper, lower):
    if any(x is None for x in [price, upper, lower]): return "Unknown"
...
def _calculate_technicals(symbol: str) -> dict:
    ...
    try:
        ...
        # ── Indicators ────────────────────────────────────────────────────────
        rsi      = _f(ta_momentum.RSIIndicator(close=close, window=14).rsi().iloc[-1])
        macd_obj = ta_trend.MACD(close=close, window_fast=12, window_slow=26, window_sign=9)
        macd_v   = _f(macd_obj.macd().iloc[-1])
        macd_sl  = _f(macd_obj.macd_signal().iloc[-1])
        macd_h   = _f(macd_obj.macd_diff().iloc[-1])
        adx      = _f(ta_trend.ADXIndicator(high=high, low=low, close=close, window=14).adx().iloc[-1])
        atr      = _f(ta_volatility.AverageTrueRange(high=high, low=low, close=close, window=14).average_true_range().iloc[-1])
        bb       = ta_volatility.BollingerBands(close=close, window=20, window_dev=2)
        bb_u     = _f(bb.bollinger_hband().iloc[-1])
        bb_m     = _f(bb.bollinger_mavg().iloc[-1])
        bb_l     = _f(bb.bollinger_lband().iloc[-1])
        e20      = _f(ta_trend.EMAIndicator(close=close, window=20).ema_indicator().iloc[-1])
        e50      = _f(ta_trend.EMAIndicator(close=close, window=50).ema_indicator().iloc[-1])
        e200     = _f(ta_trend.EMAIndicator(close=close, window=200).ema_indicator().iloc[-1])
        vol_sma  = _f(volume.rolling(20).mean().iloc[-1])
