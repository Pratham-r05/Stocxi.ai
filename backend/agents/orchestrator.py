...
Responsibilities:
  - Parallel fan-out to all 5 data agents (20s per-agent timeout)
...
async def run(request: FetchRequest) -> tuple[AnalysisResult, dict[str, Any]]:
    ...
    Steps:
      1. Parallel fan-out → 5 data agents (20s timeout each)
      2. AnonMap construction + scrub sanitized=False nodes
      3. INSUFFICIENT_DATA gate (raises InsufficientDataError if failing)
      4. Cache lookup — return early on hit
    ...
    # ── 3. Cache lookup (before gate — serve cached result even if today's fetch is partial) ──
    ...
    _check_sufficient(all_nodes)   # raises InsufficientDataError if needed
    ...
    draft, full_prompt, full_raw_output = await agent_analysis.run(all_nodes, request)
    ...
    result, admin_view = formatter.format_result(
        ...
        request=request,
        nodes=all_nodes,
        failed_fetches=failed_msgs,
        analysis_id=analysis_id,
        latency_ms=latency_ms,
        cache_hit=False,
    )

    # ── 5b. Apply confidence calibration ───────────────────────────────────────
    # Maps raw_confidence (from LLM) → calibrated_confidence (backtest-informed).
    # Identity map when no backtest data exists yet — pass-through.
    ...
    log_analysis(
        analysis_id=analysis_id,
        stock=request.stock,
        profile_bucket=request.profile.bucket,
        as_of_date=str(request.as_of_date),
        node_ids=node_ids,
        prompt_version=draft.prompt_version,
        weight_version=draft.weight_version,
        model_id=draft.model_id,
        input_nodes_json=json.dumps([n.node_id for n in all_nodes]),  # IDs only — no bloat
    ...
    # ── 8. 3D Knowledge graph — saved per stock per date ───────────────────────
    try:
        ...
        graph_title = f"Stocxi — {request.stock} | {graph_date}"
        render_3d_html(G, title=graph_title, output_path=graph_path)
        admin_view["knowledge_graph_path"] = str(graph_path)
        logger.info("orchestrator: 3D graph saved → %s (%d nodes, %d edges)",
                    graph_path, G.number_of_nodes(), G.number_of_edges())
    except Exception as graph_exc:
        logger.warning("orchestrator: knowledge graph generation failed — %s", graph_exc)
